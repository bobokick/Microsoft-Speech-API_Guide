...
* `PHRASE`/`P`标签
  短语类标签，用于包含一个或多个短语，但通常只包含一个短语。
  一个`PHRASE`或者`P`标签可以有自己的短语文本，还可以包含各种短语类标签，对于规则来说，该标签表示其包含的所有短语所组成的长短语，也就是语音中这些短语都能被识别。

  `PHRASE`或者`P`标签通常包含：
  * 一个文本内容，表示该标签所包含的一个短语，该短语在规则中被SR引擎所识别。
  * 零个或多个`PHRASE`/`P`标签、`LIST`/`L`标签、`RULEREF`标签或者`OPT`/`O`标签，这些标签除了遵循自身的规则外，其所表示的短语都可能会被SR引擎所识别。
  * 可以有各种语义属性，也就是`PROPNAME`、`PROPID`、`VALSTR`和`VAL`属性（其中`PROPNAME`和`PROPID`属性可以单独或一起使用；`VALSTR`或`VAL`属性不能单独使用，必须要和`PROPNAME`和`PROPID`属性的任意一个或全部一起使用），分别表示该标签所含有的一些信息（这些属性储存于[`SPPHRASEPROPERTY`结构体](https://docs.microsoft.com/en-us/previous-versions/windows/desktop/ee125126(v=vs.85))中）。
  ...
  <!-- 主规则rule3，含有三个PHRASE和一个LIST标签，
  该语法规则能识别以下语句：
  1. "今天我要学习语文"
  2. "今天我要学习数学"
  3. "今天我要学习英语"
  -->
  <RULE NAME="rule3" TOPLEVEL="ACTIVE">
      <P PROPNAME="ph2" PROPID="1" VALSTR="today" VAL="30">今天</P>
      <P PROPID="2">我要</P>
      <P PROPID="1" VALSTR="study">
          学习
...
* `LIST`/`L`标签
  短语类标签，一个`LIST`或者`L`标签也表示语法规则中的一个短语，不过它自己不能包含短语文本，只包含各种短语类标签，并用其所含的短语类标签中的任意一个短语来表示自己的短语。
